...
def test_format_security_warning_with_cve_id():
    ...
    warning = format_security_warning(msg, lineno, cve_id)
    assert warning == (
...
def test_format_security_warning_without_cve_id():
    msg = "Unsafe eval usage detected."
    lineno = 42
    warning = format_security_warning(msg, lineno)
    assert warning == (f"{SECURITY_LABEL} Line {lineno}: {msg}")


def test_format_optimization_warning():
    msg = "Unused import detected."
    lineno = 10
    warning = format_optimization_warning(msg, lineno)
    assert warning == (f"{OPTIMIZATION_LABEL} Line {lineno}: {msg}")
