...
def check_deeply_nested_loops(tree: ast.AST, max_depth: int = 2) -> List[str]:
    """
    Detect deeply nested loops (for/while) that exceed the specified nesting depth.
    High nesting depth can indicate complex code that may be hard to maintain and 
    ...
    Returns:
        List of warning messages for loops exceeding the nesting depth
    """
    issues: List[str] = []
    ...
    class LoopNestingVisitor(ast.NodeVisitor):
        ...
        def visit_AsyncFunctionDef(self, node: ast.AsyncFunctionDef):
            # Reset depth for each async function
            prev_depth = self.current_depth
            self.current_depth = 0
            self.generic_visit(node)
            self.current_depth = prev_depth
        ...
        def visit_For(self, node: ast.For):
            ...
            if self.current_depth > self.max_depth:
                issues.append(
                    format_optimization_warning(
                        f"High complexity: Loop nesting depth is {self.current_depth}. "
    ...
    LoopNestingVisitor(max_depth).visit(tree)
    return issues
...
def run_all_optimization_checks(source_code: str) -> List[str]:
    ...
    issues.extend(check_deeply_nested_loops(tree))  # New check added here
